...
void PlaySideBar::AddFlag(std::string filename, float width, int value) {
	...
	mFlagTextures.push_back(new TextureGL(filename, x, 0, (int)width - 2, 64));
...
PlaySideBar::PlaySideBar() {
	...
	mBackground = new TextureGL("Black.png");
	mBackground->Parent(this);
	mBackground->Scale(Vector2(3.0f, 10.0f));
	mBackground->Position(45.0f, 380.0f);

	mHighLabel = new TextureGL("HIGH", "emulogic.ttf", 32, { 150, 0, 0 });
	mHighLabel->Parent(this);
	mHighLabel->Position(-25.0f, 0.0f);

	mScoreLabel = new TextureGL("SCORE", "emulogic.ttf", 32, { 150, 0, 0 });
	...
	mPlayerOneLabel = new TextureGL("1UP", "emulogic.ttf", 32, { 150, 0, 0 });
	...
	for (int i = 0; i < MAX_SHIP_TEXTURES; ++i) {
		mShipTextures[i] = new TextureGL("PlayerShips.png", 0, 0, 60, 64);
